...
export class PerplexityClient {
      ...
      messages: [
        {
          ...
          content:
            systemPrompt +
            '\n\nIMPORTANT: The user will provide actual URLs below. Do NOT search online. Analyze ONLY the provided URLs content and extract professional experiences in the EXACT format "Title @Company | Duration". Focus on work experience, education, and professional background. If information is not available in the provided content, leave those fields empty.',
        },
        {
          role: "user",
          content: userPrompt,
  ...
  async analyzeProfile(systemPrompt: string, userPrompt: string): Promise<any> {
    ...
    console.log(
      "Request body prepared for LinkedIn analysis via Perplexity API"
    );
    ...
    console.log(
      "Perplexity API response status for LinkedIn analysis:",
      response.status
    );
